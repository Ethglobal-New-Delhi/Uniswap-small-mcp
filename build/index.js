...
class UniswapMCPServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    case "connect_wallet":
                        return await this.connectWallet(args.privateKey);
                    case "get_balance":
                        return await this.getBalance(args.address, args.tokenAddress);
                    case "get_quote":
                        return await this.getQuote(args.tokenIn, args.tokenOut, args.amountIn, args.fee || 3000);
                    case "execute_swap":
                        return await this.executeSwap(args.tokenIn, args.tokenOut, args.amountIn, args.minAmountOut, args.fee || 3000, args.slippagePercent || 1);
                    case "approve_token":
                        return await this.approveToken(args.tokenAddress, args.amount || "max");
    ...
    async getQuote(tokenIn, tokenOut, amountIn, fee) {
        try {
            ...
            const quote = await quoterContract.quoteExactInputSingle.staticCall(tokenIn, tokenOut, fee, amountInWei, 0);
    ...
    async executeSwap(tokenIn, tokenOut, amountIn, minAmountOut, fee, slippagePercent) {
        if (!this.wallet) {
